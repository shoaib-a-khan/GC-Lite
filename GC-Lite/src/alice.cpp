...
void Alice::RunGCProtocol()
{
    ...
    printf("Communication Cost = %ld bytes\n", communication_cost);
}
...
void Alice::ObliviousExecute()
{
    ...
    static int count = 0;
    // std::cout << "Alice: Oblivious Execute Run # " << count++ << "\n";
    // std::cout << "Alice: pc = " << pc << "\n";
    /*------- Step 1 -------*/
    Message msg_for_Bob, msg_from_Bob;
    msg_for_Bob.scalar1 = P[pc][0];
    msg_for_Bob.scalar2 = P[pc][1];
    srv->Send(msg_for_Bob);
    communication_cost += sizeof(msg_for_Bob);
    // std::cout << "Alice: Msg sent to Bob : " << msg_for_Bob.scalar1 << " " << msg_for_Bob.scalar2 << "\n";
    msg_from_Bob = srv->Receive();
    // std::cout << "Alice: Msg rcvd from Bob: " << msg_from_Bob.scalar1 << " " << msg_from_Bob.scalar2 << "\n";
    /*------- Step 2 -------*/
    long a = P[pc][0] + msg_from_Bob.scalar1;
    long b = P[pc][1] + msg_from_Bob.scalar2;
    // std::cout << "Alice: a = " << a << " b = " << b << "\n";
    if (a < 0 || b < 0)
    {
    ...
    D[a][0] = D[a][0] - D[b][0];
    output = D[a][0];
    // std::cout << "Alice: output in this round = " << output << "\n";
    /*------- Step 4 -------*/
    long beta = ObliviousComparison(">=", 0, D[a][0]);
    // std::cout << "Alice: OC output  beta' = " << beta << "\n";
    /*------- Step 5 -------*/
    double next_pc = ObliviousMultiplication(beta, P[pc][2]);
    // std::cout << "Alice: OM output beta'* P'[pc][2] : " << std::fixed << next_pc << "\n";
    double temp = ObliviousMultiplication(1 - beta, P[pc][3]);
    // std::cout << "Alice: OM output (1-beta')(P'[pc][3]) : " << std::fixed << temp << "\n";
    pc = next_pc + temp;
    // std::cout << "Alice's pc : " << pc << "\n";
...
void Alice::PermuteProgram()
{
    ...
    communication_cost += (n_p * sizeof(Instruction));
}
...
void Alice::PermuteData()
{
    ...
    srv->SendDataVector(V, n_d); //sending Bob his BlindPermuted share of D
    communication_cost += (n_d * sizeof(DataElement));
}
...
void Alice::BlindPermuteData()
{
    ...
    // std::cout << "Alice's Permuted Data D_B:\n";
    for (int i = 0; i < n_d; i++)
    {
        for (int j = 0; j < 2; j++)
        {
