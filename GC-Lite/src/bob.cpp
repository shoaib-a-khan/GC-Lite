...
void Bob::ObliviousExecute()
{
    ...
    static int count = 0;
    // std::cout<<"Bob: Oblivious Execute Run # "<<count++<<"\n";
    // std::cout<<"Bob: pc = "<<pc<<"\n";
    /*------- Step 1 -------*/
    ...
    // std::cout<<"Bob: Msg rcvd from Alice : "<<msg_from_Alice.scalar1 << " "<<msg_from_Alice.scalar2<<"\n";
    cli->Send(msg_for_Alice);
    // std::cout<<"Bob: Msg sent to Alice : "<<msg_for_Alice.scalar1 << " "<<msg_for_Alice.scalar2<<"\n";
    /*------- Step 2 -------*/
    long a = P[pc][0] + msg_from_Alice.scalar1;
    long b = P[pc][1] + msg_from_Alice.scalar2;
    // std::cout<<"Bob: a = "<<a<<" b = "<<b<<"\n";
    if(a<0 || b<0){
    ...
    D[a][0] = D[a][0] - D[b][0];
    output = D[a][0];
    // std::cout<<"Bob: output in this round = "<<output<<"\n";
    /*------- Step 4 -------*/
    long beta = ObliviousComparison(">=", 0, D[a][0]);
    // std::cout<<"Bob: OC output  beta\" = "<<beta<<"\n";
    /*------- Step 5 -------*/
    double next_pc = ObliviousMultiplication(beta, P[pc][2]);
    // std::cout<< "Bob: OM output beta\"* P\"[pc][2] : "<<std::fixed<<next_pc<<"\n";
    double temp = ObliviousMultiplication(-beta, P[pc][3]);
    // std::cout<< "Bob: OM output (-beta\")(P\"[pc][3]) : "<<std::fixed<<temp<<"\n";
    pc = next_pc + temp;
    // std::cout<<"Bob's pc : "<<pc<<"\n";    
...
void Bob::PermuteData()
{
    ...
    cli->SendDataVector(V, n_d); //sending Alice her BlindPermuted share of D
}
